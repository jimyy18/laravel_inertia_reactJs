...
export default function Create({user}) {
  ...
  const handleSubmit=async()=>{
    try {
      const createUsers = await createUser(formData);

      console.log(createUsers,'createUsers')
  
      // Memeriksa apakah update berhasil
      if (createUsers?.message === "success") {
        // Jika berhasil, alihkan ke halaman dashboard
        window.location.href = '/dashboard';
      } else {
        console.error("Update failed: ", createUsers.message);
      }
    } catch (error) {
      console.error("Error saat Tambag:", error);
    }
